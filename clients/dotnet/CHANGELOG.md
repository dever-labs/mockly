...
- `SetFaultAsync` now correctly calls `POST /api/fault/http` (protocol-scoped endpoint); the old `/api/fault` path does not accept POST.
- `ClearFaultAsync` correctly handles the server's `204 No Content` response.
